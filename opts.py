...
def configure_args():
    ...
    parser.add_argument('--save', type=str, default='lang_model.pt', help='path for best .pt checkpoint')
    ...
    parser.add_argument('--noise_r', type=float, default=0.05, help='std of noise for autoencoder')
    ...
    parser.add_argument('--kenlm_model', type=str, required=True, help='path to reference kenlm model for computing forward ppl')
    ...
    # ParalARAE params
    choices = ['lstm', 'dense', 'dense_pos', 'conv']
    parser.add_argument('--dec_type', nargs='?', default='lstm', const='lstm', choices=choices, help='decoder type')
